...
var Utility = {};
Utility.prototype.closestPointOnCircle = function(circlex,circley,circlerad,ptx,pty) {
  var dx = ptx - circlex;
  var dy = pty - circley;
  var scale = Math.sqrt(dx * dx + dy * dy);
  return {
    'x': circlex + dx * circlerad / scale,
    'y': circley + dy * circlerad / scale
  };
};
Utility.prototype.circleContainsPoint = function(circlex,circley,circlerad,ptx, pty) {
  return (ptx - circlex)*(ptx - circlex) + (pty - circley)*(pty - circley) < circlerad*circlerad;
};